    ...
    <div v-for="set of yamlContent.sets" :key="set.name" mb-6 rounded bg-white p-4 shadow-md>
      ...
      <div v-for="rule of set.rules" :key="rule.name" my-2 rounded bg-blue-100 p-2>
        <div flex items-start>
          <img :src="get_image_path([set.name, `${set.name}_rules`, rule.name])" :alt="rule.name"
      ...
      <div v-for="hero of set.heroes" :key="hero.name" my-4 b-2 rounded bg-gray-50 p-2>
        <div flex items-start>
          <img :src="get_image_path([set.name, hero.name, hero.name])" :alt="hero.name" mx-2 mt-2
               h-32 rounded object-contain>
          <div>
            <h3 text-primary-700 font-bold> {{ hero.name }} - {{ hero.health }} health,
              {{ hero.speed }} speed</h3>
            <pre>{{ hero.description }}</pre>

            <p v-if="hero.passives">---</p>
            ...
            <p v-if="hero.default_abilities">---</p>
        ...
        <div v-for="ability of hero.abilities" :key="ability.name" my-2 rounded bg-blue-100 p-2>
          <div class="flex items-start">
            <img :src="get_image_path([set.name, hero.name, ability.name])" :alt="ability.name" mx-2
        ...
        <div v-for="unit of hero.units" :key="unit.name" my-2>
          <div class="flex items-start">
            <img :src="get_image_path([set.name, hero.name, unit.name])" :alt="unit.name" mx-2 mt-2
                 h-32 rounded object-contain>
            <h4 class="text-primary-600 font-bold">
              {{ unit.name }}
